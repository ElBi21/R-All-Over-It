...
Segmentation is, in poor words, the act of **separating** and **finding** groups that would include one or multiple elements of a dataset. There are multiple methods that allow us to accomplish segmentation, such as **clustering** and **classification**.

## 1) Clustering

Clustering is the action of **finding groups within** an enormous **set of data**. The problem with clustering is that it's hard to **determine** over which **variables** the data should be clustered, it's hard to **find** the **right number of clusters**, and it's also hard to ensure that the cluster has a valuable meaning. In `R` it's possible to do it thanks to some built-in functions. First, let's see the toy dataset that will be used for explaining what clustering means:
...
How many clusters can we have for a dataset? In the **worst case** scenario, **as many as the points** inside the dataset. We have to be more precise and avoid such scenario. Let's try to peak at the density of the `seg.raw$income` variable, and see if we can get some useful insights for our analysis:

```{r}
plot(density(seg.raw$income))
```

We notice two peaks: this tells us that, if we wanted to cluster the data depending on the `seg.raw$income` variable, then we could possibly have two main clusters. Now, we want to put into the `seg.raw$Segment` variable the "label" of the cluster. Since the dataset has already such variable, we'll proceed to remove it.
...
In order to better observe the data, we create a simple function which will return the mean values by group. All the qualitative variables will be replaces with `NA`s.
